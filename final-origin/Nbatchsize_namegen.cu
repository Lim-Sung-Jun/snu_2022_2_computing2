...
void embedding(Tensor *input, Tensor *weight, Tensor *output) {
  ...
  size_t n = weight->shape[1]; // weight에 shape도 있고 buf가 있다.
  for(size_t j = 0; j < input_N; j++){
    for (size_t i = 0; i < n; i++) {
        int x = (int)input->buf[j];  // 현재 문자가 무엇인지 알아낸다.
        output->buf[j * n + i] = weight->buf[x * n + i];
    }
  }
}
...
void namegen_initialize(int N, int rng_seed, char *parameter_fname) {
  ...
  if (mpi_rank == 0) {
    ...
    input = new Tensor({N, 1});//1
    emb_out = new Tensor({N, EMBEDDING_DIM});//512
    ...
    ftmp0 = new Tensor({N, NUM_CHAR});//256
    char_prob = new Tensor({N, NUM_CHAR});//256
  } else {
  }
}
...
void namegen(int N, float *random_floats, char *output) { // 병렬화 대상
  ...
  for (int l = 0; l < MAX_LEN; l++) { // 이름의 최대 길이는 10이다.
    /* Embedding */
    embedding(input, character_embedding, emb_out); // 여기서 input은 buf[0]만을 가지고 있다. 만약 사이즈는 512다. < N,512 >
    // printf("shape of output: %zd, %zd\n", emb_out->shape[0], emb_out->shape[1]);
    // for(size_t i = 1; i < 2; i++){
    //   printf("\n");
    //   for(size_t j = 0; j < 3; j++){
    //     printf("the value of emb_out[%zd][%zd]: %f\n", i ,j , character_embedding->buf[i * 512 + j]);
    //   }
    // }
  ...
  //   elemwise_add(rtmp00, b_ir0, rtmp02); // weight_input * embedding + bias_input / -> modify function
