...
void namegen(int N, float *random_floats, char *output) { // 병렬화 대상
  ...
  if (mpi_rank == 0){
    ...
    /* Generate N names */
    for (int n = size * mpi_rank; n < size * mpi_rank + size; n++) {
      // printf("n = %d, full = %d\n", size * mpi_rank, size * mpi_rank + size);
    // for (int n = 0; n < N; n++) {
    ...
    // 이제 계산을 되는데 시발 receieve가 안되네;;;;;;??????
    // char* output_buff = (char *)malloc(N * (MAX_LEN + 1) * sizeof(char));
    // #pragma omp parallel for num_threads(mpi_size - 1)
    for(int i = 1; i < mpi_size; i++)
    {
      int M_start = i * (N / mpi_size);
      ...
      printf("[mpi_rank:%d] recv address:%d\n",mpi_rank, M_start * (MAX_LEN + 1));
      printf("[mpi_rank:%d]amount of receieved message: %d\n", mpi_rank,(M_end - M_start) * (MAX_LEN + 1));
      MPI_Recv((void*)(output + M_start * (MAX_LEN + 1)),  (M_end - M_start) * (MAX_LEN + 1), MPI_CHAR, i, i, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    // memcpy(output + (MAX_LEN + 1), output_buff + (MAX_LEN + 1), (N - 1)*(MAX_LEN + 1)*sizeof(char));
  }
  else if(mpi_rank != 0){
    char* output_buff = (char *)malloc(N * (MAX_LEN + 1) * sizeof(char));
    char* random_floats_buff = (char *)malloc(N * (MAX_LEN) * sizeof(float));
    MPI_Recv((void*)(output_buff + M_start * (MAX_LEN + 1)),  (M_end - M_start) * (MAX_LEN + 1), MPI_CHAR, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    // for (int n = 0; n < N; n++) {
      ...
      for (int l = 0; l < MAX_LEN; l++) { // 이름의 최대 길이는 10이다.
        ...
        /* Random select */
        int selected_char = random_select(char_prob, rfloats, n * MAX_LEN + l);
        // printf("mpi_rank:%d, selected_char:%d\n", mpi_rank, selected_char);

        // 여기서 에러가 발생하는 이유는 뭐지? output -> output_buff
        output_buff[n * (MAX_LEN + 1) + l] = selected_char;
        input->buf[0] = selected_char; // 다시 입력으로 넣는다.
    ...
    MPI_Send(output_buff + M_start * (MAX_LEN + 1), (M_end - M_start) * (MAX_LEN + 1), MPI_CHAR, 0, mpi_rank, MPI_COMM_WORLD);
  ...
  //   {
  //     int M_start = i * (N / mpi_size);
  //     MPI_Recv(output + M_start * (MAX_LEN + 1), size * (MAX_LEN + 1), MPI_CHARACTER, i,i , MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  //     printf("[mpi_rank:%d] recv address:%d\n",mpi_rank, M_start * (MAX_LEN + 1));
  //   }
  // }
  // }else{
  //   MPI_Send(output_buff + M_start * (MAX_LEN + 1), size * (MAX_LEN + 1), MPI_CHARACTER, 0, mpi_rank, MPI_COMM_WORLD);
  //   printf("[mpi_rank:%d] send address:%d\n",mpi_rank, M_start * (MAX_LEN + 1));
  // }
}
